...
Процедура ВыполнитьДействие(Команда)
	...
	Если стрКоманда="Выбрать" Тогда
		Если ИспользоватьКорзину Тогда
			стрДанные="";
			Для каждого СтрокаКоллекции Из тзКорзина Цикл
				стрДанные=стрДанные+?(ПустаяСтрока(стрДанные), "", Символы.ПС)+СтрокаКоллекции.Идентификатор;
			КонецЦикла;			
			ЭтаФорма.Закрыть(стрДанные);
		Иначе	
...
Процедура ДеревоРеквизитов_Выбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если ИспользоватьКорзину Тогда
		Если Не Элементы.ДеревоРеквизитов.ТекущиеДанные=Неопределено И Элементы.ДеревоРеквизитов.ТекущиеДанные.ДоступностьВыбора Тогда
			ТекущиеДанные=Элементы.ДеревоРеквизитов.ТекущиеДанные;
			стрИдентификатор=?(ВозвращатьКраткоеИмя, ТекущиеДанные.КраткоеИмя, ТекущиеДанные.ПолноеИмя);
			МассивСтрок=тзКорзина.НайтиСтроки(Новый Структура("Идентификатор", стрИдентификатор));
			Если НЕ МассивСтрок.Количество()=0 Тогда Возврат; КонецЕсли;	 
			НоваяСтрока=тзКорзина.Добавить();
			НоваяСтрока.Идентификатор=стрИдентификатор;
			НоваяСтрока.Представление=ТекущиеДанные.ПолноеИмя;			
		КонецЕсли;
	Иначе
...
Функция ПолучитьЗначениеНастройки(СтрокаДерева, СтрокаНастройки)
	...
	ИначеЕсли ТипЗнч(СтрокаНастройки)=Тип("ОписаниеТипов") Тогда
		Для Каждого ТипВНастройке Из СтрокаНастройки.Типы() Цикл
			Если СтрокаДерева.ТипЗначения.СодержитТип(ТипВНастройке) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
...
Процедура ИнициализироватьКартинку(СтрокаДерева, ЭтоКорневойЭлемент)
	...
	Если ЭтоКорневойЭлемент Тогда
		...
		Иначе
			...
			ИначеЕсли СтрНачинаетсяС(СтрокаДерева.Имя, "Документ") Тогда
