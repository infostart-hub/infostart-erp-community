...
Процедура ТабличноеПоле_Инициализация()
	...
	ПолеТабличногоДокумента.Области.ВесоваяЕдиница.Очистить();
...
Процедура ТабличноеПоле_ОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТекущаяОбласть.Низ=ТекущаяОбласть.Верх Тогда
		...
		Если СтруктураПараметров.Ключ.Пустая() Тогда
			...
			СтруктураПараметров.Вставить("МеждународноеСокращение", ПолеТабличногоДокумента.Область(ТекущаяОбласть.Низ, 7).Текст);

			ОписаниеОповещения=Новый ОписаниеОповещения("ОбработчикОповешения_ОтветНаВопрос", ЭтотОбъект, СтруктураПараметров);
			стрВопрос="Записать в справочник ""Классификатор ОКЕИ"" элемент с наименование """+СтруктураПараметров.Наименование+"""?";
		...
		Иначе
			ОткрытьФорму("Справочник.КлассификаторЕдиницИзмерения.Форма.ФормаЭлемента", СтруктураПараметров, ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
