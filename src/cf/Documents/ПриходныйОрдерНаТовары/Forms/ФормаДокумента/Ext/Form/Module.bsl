...
Процедура УстановитьВидимостьЭлементовФормы()
	// Физ. лицо нужно показывать только для операций поступления товаров, принятых от подотчетника.
	Элементы.ФизЛицо.Доступность=Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходныйОрдер.ОтПодотчетника");

	ЭтоОперацияПеремещения=Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходныйОрдер.Перемещение");

	Элементы.ДокументПеремещения.Доступность=ЭтоОперацияПеремещения;
	Элементы.Контрагент.Доступность=НЕ ЭтоОперацияПеремещения;
	Элементы.БезПраваПродажи.Доступность=НЕ ЭтоОперацияПеремещения;

	
	Элементы.ТоварыДокументРезерва.Доступность=Не ЭтоОперацияПеремещения;
	Элементы.ВозвратнаяТараДокументРезерва.Видимость=Не ЭтоОперацияПеремещения;

	// Качество указывается только для брака
	Элементы.ТоварыКачество.Видимость=ЭтоОперацияПеремещения	
КонецПроцедуры

&НаКлиенте
Процедура ВыборВидаОперации(Команда)
	Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходныйОрдер."+стрЗаменить(Команда.Имя, "Операция_", ""));

	Если Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходныйОрдер.Перемещение") Тогда
		Объект.Контрагент=Неопределено;
		Объект.БезПраваПродажи=Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработчикОповещения_ЗаполнитьПоЗаказуПоставщику(Параметр1, Параметр2) Экспорт
	Если Параметр1=Неопределено Тогда Возврат; КонецЕсли;
	Параметр2.Вставить("ДокументОснование", Параметр1);
	тпТовары_Заполнить(Параметр2.Команда, Параметр2);
КонецПроцедуры
...
Процедура ОбработчикОповещения_Товары_Заполнить(Параметр1, Параметр2) Экспорт
	...
	Если Параметр2.Команда="ЗаполнитьПоЗаказуПоставщику" Или  Параметр2.Команда="ЗаполнитьПоЗаказуПоставщикуПоОстаткам" Тогда
		Параметр2.Вставить("ИмяТабличнойЧасти", "Товары");
		ПоказатьВводЗначения(Новый ОписаниеОповещения("ОбработчикОповещения_ЗаполнитьПоЗаказуПоставщику", ЭтотОбъект, Параметр2), , "Выберите заказ поставику для заполнения приходного ордера", Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьПоЗаказуПоставщику(стрКоманда, СтруктураПараметров)
	Если стрКоманда="ЗаполнитьПоЗаказуПоставщикуПоОстаткам" Тогда //с учетом остатков
		...
		Если СтруктураПараметров.ИмяТабличнойЧасти="Товары" Тогда
			МассивХарактеристикНоменклатуры=Объект.Товары.Выгрузить(, "ХарактеристикаНоменклатуры");
		...
		Запрос.УстановитьПараметр("ИсточникДанных", Объект[СтруктураПараметров.ИмяТабличнойЧасти].Выгрузить());
		Запрос.УстановитьПараметр("НаДату", Объект.Дата);
		Запрос.УстановитьПараметр("Организация", Объект.Организация);
		Запрос.УстановитьПараметр("Заказ", СтруктураПараметров.ДокументОснование);
		Запрос.УстановитьПараметр("Номенклатура", Объект[СтруктураПараметров.ИмяТабличнойЧасти].Выгрузить(, "Номенклатура"));
		...
		Для каждого СтрокаКоллекции Из тзДвижения Цикл
			...
			НоваяСтрока=ЭтотОбъект[СтруктураПараметров.ИмяТабличнойЧасти].Добавить();
	...
	Иначе
		...
		|	
		|ИЗ
		|	Документ.ЗаказПоставщику."+СтруктураПараметров.ИмяТабличнойЧасти+" КАК ИсточникДанных
		|ГДЕ
		|	ИсточникДанных.Ссылка = &ДокументОснование
		|";
		Выборка=Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(Объект[СтруктураПараметров.ИмяТабличнойЧасти].Добавить(), Выборка);
		КонецЦикла;
	КонецЕсли;
...
Процедура Атрибут_ПриИзменении(Элемент)
    Если Элемент.Имя="ДокументПеремещения" Тогда
		Если ЗначениеЗаполнено(Объект.ДокументПеремещения) Тогда
			ДанныеДокументаПеремещения=ОбщегоНазначенияСервер.ЗначенияРеквизитовОбъекта(Объект.ДокументПеремещения, "Организация,СкладПолучатель");
			Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
				Объект.Организация=ДанныеДокументаПеремещения.Организация;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Объект.Склад) Тогда
				Объект.Склад=ДанныеДокументаПеремещения.СкладПолучатель;
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли;
КонецПроцедуры
...
Процедура Атрибут_НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Элемент.Имя="ДокументПеремещения" Тогда
		...
		СтруктураОтбора.Вставить("ВидОперации", ПредопределенноеЗначение("Перечисление.ВидыОперацийПеремещениеТоваров.ТоварыПродукцияПоОрдерам"));

		ПараметрыФормы=Новый Структура;
		ПараметрыФормы.Вставить("Отбор", СтруктураОтбора);
		ПараметрыФормы.Вставить("РежимВыбора", Истина);

		ОткрытьФорму("Документ.ПеремещениеТоваров.ФормаВыбора", ПараметрыФормы, Элемент, УникальныйИдентификатор,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
...
Процедура кпВозвратнаяТара_ВыполнитьДействие(Команда)
	...
	Если стрКоманда="ЗаполнитьПоДокументуПередачи" Тогда
		//Если НЕ ЗначениеЗаполнено(Объект.ДокументПеремещения) Тогда
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите документ передачи",, "Объект.ДокументПеремещения");
		//	Возврат;
		//КонецЕсли;
...
Процедура тпВозвратнаяТара_ЗаполнитьПоДокументуПередачи()
	...
	//Запрос.УстановитьПараметр("ДокументОснование", Объект.ДокументПеремещения);
	...
	//	|	Остатки.КоличествоОстаток КАК Количество
	//	|ИЗ
	//	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки(, ДокументПеремещения = &ДокументОснование И Склад = &Склад И СтатусПартии = &СтатусПартии) КАК Остатки
	//	|";
	//КонецЕсли;
	//Выборка=Запрос.Выполнить().Выбрать();
	//Пока Выборка.Следующий() Цикл
...
Процедура кпТовары_ВыполнитьДействие(Команда)
	...
	Иначе
		...
		Если НЕ Объект.Товары.Количество()=0 Тогда
			ОписаниеОповещения=Новый ОписаниеОповещения("ОбработчикОповещения_Товары_Заполнить", ЭтотОбъект, СтруктураПараметров);
			ПоказатьВопрос(ОписаниеОповещения, "Очистить табличную часть перед заполнением?", РежимДиалогаВопрос.ДаНетОтмена, 20, КодВозвратаДиалога.Отмена, "Внимание", КодВозвратаДиалога.Отмена);
		Иначе
			Если стрКоманда="ЗаполнитьПоЗаказуПоставщику" Или  стрКоманда="ЗаполнитьПоЗаказуПоставщикуПоОстаткам" Тогда				
				ПоказатьВводЗначения(Новый ОписаниеОповещения("ОбработчикОповещения_ЗаполнитьПоЗаказуПоставщику", ЭтотОбъект, СтруктураПараметров), , "Выберите заказ поставику для заполнения приходного ордера", Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
				Возврат;
			КонецЕсли;
			тпТовары_Заполнить(стрКоманда);
		КонецЕсли;
...
Процедура тпТовары_ЗаполнитьПоДокументуПередачи()
	...
	//|	Остатки.КоличествоОстаток КАК Количество
	//|ИЗ
	//|	РегистрНакопления."+стрРегистр+".Остатки(, ДокументПеремещения = &ДокументОснование И Склад = &Склад И СтатусПартии = &СтатусПартии) КАК Остатки
	//|";		
	//Выборка=Запрос.Выполнить().Выбрать();
	//Пока Выборка.Следующий() Цикл
	//	Если Выборка.Количество=0 ИЛИ Выборка.Количество=NULL Тогда Продолжить; КонецЕсли;
...
&НаСервере
Процедура тпТовары_Заполнить(стрКоманда, СтруктураПараметров=Неопределено)
	Если стрКоманда="ЗаполнитьПоСериям" Тогда
		ДокументОбъект=РеквизитФормыВЗначение("Объект");
		ОбработкаТабличныхЧастей.ЗаполнитьСерии(ДокументОбъект);
		ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");

	ИначеЕсли стрКоманда="ЗаполнитьПоЗаказуПоставщику" Или  стрКоманда="ЗаполнитьПоЗаказуПоставщикуПоОстаткам" Тогда
		ЗаполнитьПоЗаказуПоставщику(стрКоманда, СтруктураПараметров);

	ИначеЕсли стрКоманда="ЗаполнитьПоДокументуПередачи" Тогда
		Если НЕ ЗначениеИзФайла(Объект.ДокументПеремещения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не заполнен документ перемещения",,"Объект.ДокументПеремещения");
		КонецЕсли;
		...
		|	-Остатки.КоличествоОстаток КАК Количество
		|ИЗ
		|	РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки(, ДокументПолучения = &ДокументОснование И СтатусПартии = &СтатусПартии И Склад = &Склад) КАК Остатки
		|ГДЕ
		|	Остатки.КоличествоОстаток <> 0		
		|";
		Выборка=Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			НоваяСтрока=Объект[СтруктураПараметров.ИмяТабличнойЧасти].Добавить();
