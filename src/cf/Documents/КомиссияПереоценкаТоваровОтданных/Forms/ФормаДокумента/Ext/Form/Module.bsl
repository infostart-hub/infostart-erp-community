...
Процедура ОбработчикОповещения_Товары_Заполнить(Параметр1, Параметр2) Экспорт
	...
	Если Параметр2.Команда="ЗаполнитьОстаткамиПоДокументуПередачи" Тогда
		...
		ОткрытьФорму("Документ.РеализацияТоваровУслуг.ФормаВыбора", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
...
Процедура Атрибут_ПриИзменении(Элемент)
	Если Элемент.Имя="Контрагент" Тогда
		...
		СтруктураПраметров.Вставить("ВидДоговора", ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером"));
...
Процедура тпТовары_Колонка_ПриИзменении(Элемент)
	...
	Если стрКолонка="Номенклатура" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТекущиеДанные.СуммаСтарая) Тогда
			ТекущиеДанные.СуммаСтарая=ТекущиеДанные.Сумма;
		КонецЕсли;
...
Процедура тпТовары_Заполнить(стрКоманда, ДокументОснование=Неопределено)
	Если стрКоманда="ЗаполнитьОстаткамиПоДоговору" Тогда
		...
		|	СУММА(ИсточникДанных.СуммаВзаиморасчетовОстаток) КАК СтоимостьОстаток
		|ИЗ
		|	РегистрНакопления.ТоварыПереданные.Остатки(, ДоговорКонтрагента = &ДоговорКонтрагента И Сделка = &Сделка И СтатусПередачи = &СтатусПередачи
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяСтрока.Сумма=ОбщегоНазначения.ПересчитатьИзВалютыВВалюту(Выборка.СтоимостьОстаток,
			...
			НоваяСтрока.СуммаСтарая=НоваяСтрока.Сумма;
