...
Процедура ВидимостьЭлементовФормы()
	Видимость=?(Объект.Склад.Пустая(), Ложь, ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(Объект.Склад, "РасчетРозничныхЦенПоТорговойНаценке"));
	Элементы.ТоварыПроцентРозничнойНаценки.Видимость=Видимость;
	Элементы.ТоварыЦенаВРознице.Видимость=Видимость;
...
Процедура тпТовары_Колонка_ПриИзменении(Элемент)
	...
	ИначеЕсли стрКолонка="ПроцентРозничнойНаценки" Тогда
		Валюта=МодульВалютногоУчета.ПолучитьВалюту("Бух");
		ТипЦенРозничнойТорговли=ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(Объект.Склад, "ТипЦенРозничнойТорговли");
		СтавкаНДС=УчетНалоговСервер.СтавкаНДС(ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Номенклатура, "СтавкаНДС"));
		ТекущиеДанные.ЦенаВРознице=УправлениеРозничнойТорговлейСервер.РассчитатьРозничнуюЦену(ТекущиеДанные.Цена,
	                                                            Валюта,
	                                                            1,
	                                                            1,
	                                                            Валюта,
	                                                            ТекущиеДанные.ПроцентРозничнойНаценки,
	                                                            Истина,
	                                                            СтавкаНДС,
	                                                            ТипЦенРозничнойТорговли,
	                                                            ТекущиеДанные.ЕдиницаИзмерения,
	                                                            ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Номенклатура, "ЕдиницаХраненияОстатков"));

	ИначеЕсли стрКолонка="ЦенаВРознице" Тогда
		Если ТекущиеДанные.Цена=0 Тогда
			ТекущиеДанные.ПроцентРозничнойНаценки=0;
		Иначе
			СтавкаНДС=УчетНалоговСервер.СтавкаНДС(ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Номенклатура, "СтавкаНДС"));

			// Увеличиваем при необходимости на величину налогов (розничная цена всегда с налогами)
			ЦенаСНалогами = ЦенообразованиеСервер.ПересчитатьЦенуПриИзмененииФлаговНалогов(ТекущиеДанные.Цена, ПредопределенноеЗначение("Перечисление.СпособыЗаполненияЦен.ПоЦенамНоменклатуры"),	Истина, Истина, Истина, СтавкаНДС);

			Коэффициент=ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Номенклатура, "ЕдиницаХраненияОстатков.Коэффициент");
			ЦенаСНалогамиЗаЕдиницуХранения = ЦенаСНалогами * Коэффициент / ТекущиеДанные.Коэффициент;

			ТекущиеДанные.ПроцентРозничнойНаценки = 100 * (ТекущиеДанные.ЦенаВРознице - ЦенаСНалогамиЗаЕдиницуХранения)/ЦенаСНалогамиЗаЕдиницуХранения;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
