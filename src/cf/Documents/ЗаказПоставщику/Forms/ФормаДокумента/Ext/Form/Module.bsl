...
&НаКлиенте
Процедура тпВозвратнаяТара_РассчитатьСумму()
	ТекущиеДанные=Элементы.ВозвратнаяТара.ТекущиеДанные;
...
&НаКлиенте
Процедура ОбработчикОповещения_ТоварыЗаполнить(Параметр1, Параметр2) Экспорт
	Если Параметр1=Неопределено Тогда Возврат; КонецЕсли;
	тпТовары_Заполнить(Параметр1, Параметр2);
КонецПроцедуры

&НаСервере
Процедура тпТовары_Заполнить(Параметр1, Параметр2)
	ДокументОбъект=РеквизитФормыВЗначение("Объект");
	
	ТабличнаяЧасть=ДокументОбъект[Параметр2.ТабличнаяЧасть];
	Если Параметр1.ОчиститьТабличнуюЧастьПередЗаполнением Тогда
		ТабличнаяЧасть.Очистить();
	КонецЕсли;

	стрКоманда=Параметр1.ВариантЗаполнения;
	Если стрКоманда="ЗаполнитьПоЗаказуПокупателя" Тогда		
		ДокументОбъект.ЗаполнитьТабличнуюЧастьПоОстаткам(ТабличнаяЧасть, Параметр1.ДокументСсылка);

	//ИначеЕсли стрКоманда="ЗаполнитьПоЗаказуПокупателяКомплектующие" Тогда
		//ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяКомплектующие(Товары, Ложь);

	ИначеЕсли стрКоманда="ЗаполнитьПоВнутреннемуЗаказу" Тогда
		ДокументОбъект.ЗаполнитьТабличнуюЧастьПоОстаткамВнутреннийЗаказ(ТабличнаяЧасть, Параметр1.ДокументСсылка)
		
	//ИначеЕсли стрКоманда="ЗаполнитьПоВнутреннемуЗаказуКомплектующие" Тогда
		//ЗаполнитьТабличнуюЧастьПоВнутреннемуЗаказуКомплектующие(Товары, Ложь);

	ИначеЕсли стрКоманда="ЗаполнитьПотребностями" Тогда
		ДокументОбъект.ЗаполнитьТабличнуюЧастьПотребностями(ТабличнаяЧасть, Параметр1.ДокументСсылка);
	КонецЕсли;
...
Процедура кпТовары_ВыполнитьДействие(Команда)
	...
	Если стрКоманда="ТоварыЗаполнить" Тогда
		ОписаниеОповещения=Новый ОписаниеОповещения("ОбработчикОповещения_ТоварыЗаполнить", ЭтотОбъект, Новый Структура("ТабличнаяЧасть", стрТабличнаяЧасть));
		ОткрытьФорму("Документ.ЗаказПоставщику.Форма.ЗаполнениеТабличнойЧасти",,ЭтаФорма, УникальныйИдентификатор,,,ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
