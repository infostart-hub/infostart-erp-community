...
Процедура ДвижениеПоРегистру_РасходыПриУСН(СтруктураШД, СтруктураТД, Отказ)
	...
	Если НЕ ВидОперации=Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПоставщиком Тогда Возврат; КонецЕсли;
...
Процедура ДвижениеПоРегистру_ВзаиморасчетыСКонтрагентами(СтруктураШД, СтруктураТД, Отказ)
	...
	Если Оплачено И ДенежныеСредстваСервер.ЕстьВзаиморасчеты(ВидОперации) Тогда
		...
		Если ВидОперации=Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ПоступлениеОплатыПоПлатежнымКартам И СуммаУслуг <> 0 Тогда
			СтрокаТаблицыПлатежей=СтруктураТД.РасшифровкаПлатежа[0];
			...
			НоваяСтрока.Период=ДатаОплаты;
			НоваяСтрока.Активность=Истина;
			НоваяСтрока.Регистратор=Ссылка;
			НоваяСтрока.ВидДвижения=ВидДвиженияНакопления[?(КоэффициентСторно=1, "Расход", "Приход")];
			НоваяСтрока.ДоговорКонтрагента=СтрокаТаблицыПлатежей.ДоговорКонтрагента;
			НоваяСтрока.Организация=Организация;
			НоваяСтрока.Контрагент=Контрагент;
			НоваяСтрока.СуммаБух=СуммаУслуг;
...
Процедура ДвижениеПоРегистру_ДвиженияДенежныхСредств(СтруктураШД, СтруктураТД, Отказ)
	...
	тзДанные=Движения.ДвиженияДенежныхСредств.ВыгрузитьКолонки();	
	...
	тзДанные.ЗаполнитьЗначения(Перечисления.ВидыДвиженийПриходРасход.Приход, "ПриходРасход");
	тзДанные.ЗаполнитьЗначения(Организация, "Организация");
	тзДанные.ЗаполнитьЗначения(СчетОрганизации, "БанковскийСчетКасса");
	тзДанные.ЗаполнитьЗначения(Ссылка, "ДокументДвижения");
	тзДанные.ЗаполнитьЗначения(Подразделение, "ЦФО");
	тзДанные.ЗаполнитьЗначения(Контрагент, "Контрагент");

	Движения.ДвиженияДенежныхСредств.Загрузить(тзДанные);
КонецПроцедуры
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ДатаДвижений=?(Оплачено, ДенежныеСредстваСервер.ПолучитьДатуДвижений(Дата, ДатаОплаты, Организация), Дата);
	...
	СтруктураШД.Вставить("ДатаОплаты", ДатаДвижений);
	СтруктураШД.Вставить("ДатаДвижений", ДатаДвижений);
